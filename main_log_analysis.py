...
def combine_all_curves(df1, df2):
    """Combine all common curves from two DataFrames based on depth without summing overlapping intervals."""
    ...
    for curve_name in common_curves:
        ...
        overlapping_intervals = temp_combined_df.groupby(temp_combined_df.index).filter(
            lambda x: len(x) > 1
        )

        # Take only one value for overlapping intervals and keep all values for non-overlapping intervals
        temp_combined_df[f"{curve_name}_combined"] = temp_combined_df.apply(
            lambda row: (
                row[curve_name]
                if row.name not in overlapping_intervals.index
                else temp_combined_df[curve_name][row.name].iloc[0]
            ),
            axis=1,
        )
...
def get_standard_scale(curve_name):
    """Get the standard scale for a given curve name."""
    if "GR" in curve_name:
        return 0, 150
    elif "SP" in curve_name:
        return -100, 100
    elif "NPHI" in curve_name:
        return 0.45, -0.15
    elif "RHOB" in curve_name:
        return 1.65, 2.95
    elif "RESISTIVITY" in curve_name:
        return 0.2, 2000
    else:
        return None, None
...
def las_header(las_file):
    ...
    if not las_file:
        ...
        for item in las_file.well:
            st.sidebar.write(f"<b>{item.descr.capitalize()} ({item.mnemonic}):</b> {item.value}", unsafe_allow_html=True)
...
def missing(las_file, well_data):
    ...
    if not las_file:
        ...
        if len(curves) <= 1:
            ...
            fig.add_trace(go.Scatter(x=data_not_nan[curves[0]], y=well_data.index, fill='tozerox', line=dict(width=0), fillcolor='#D3CBCB', showlegend=True, name='Complete'), row=1, col=1)
            fig.add_trace(go.Scatter(x=data_nan[curves[0]], y=well_data.index, fill='tozerox', line=dict(width=0), fillcolor='#FF807E', showlegend=True, name='Missing'), row=1, col=1)
            for curve in curves[1:]:
                fig.add_trace(go.Scatter(x=data_not_nan[curve], y=well_data.index, fill='tozerox', line=dict(width=0), fillcolor='#D3CBCB', showlegend=False), row=1, col=curve_index)
                fig.add_trace(go.Scatter(x=data_nan[curve], y=well_data.index, fill='tozerox', line=dict(width=0), fillcolor='#FF807E', showlegend=False), row=1, col=curve_index)
...
def plot(las_file, well_data, zones, cutoffs):
    ...
    if not las_file:
        ...
        with st.expander('Log Plot'):
            st.write("Select the curves you want to plot. You can select up to three curves to be plotted on the same track.")
            ...
            for i in range(1, num_tracks + 1):
                with st.container():
                    primary_curve = st.selectbox(f'Select Primary Curve for Track {i}', columns, key=f'primary_{i}')
                    secondary_curve = st.selectbox(f'Select Secondary Curve for Track {i} (optional)', ['None'] + columns, key=f'secondary_{i}')
                    tertiary_curve = st.selectbox(f'Select Tertiary Curve for Track {i} (optional)', ['None'] + columns, key=f'tertiary_{i}')

                    x_axis_min = st.number_input(f'Set Minimum X-Axis Value for {primary_curve}', value=well_data[primary_curve].min(), key=f'min_{primary_curve}_{i}')
                    x_axis_max = st.number_input(f'Set Maximum X-Axis Value for {primary_curve}', value=well_data[primary_curve].max(), key=f'max_{primary_curve}_{i}')

                    if secondary_curve != 'None':
                        x_axis_min_sec = st.number_input(f'Set Minimum X-Axis Value for {secondary_curve}', value=well_data[secondary_curve].min(), key=f'min_{secondary_curve}_{i}')
                        x_axis_max_sec = st.number_input(f'Set Maximum X-Axis Value for {secondary_curve}', value=well_data[secondary_curve].max(), key=f'max_{secondary_curve}_{i}')
                    else:
                        x_axis_min_sec = None
                        x_axis_max_sec = None

                    if tertiary_curve != 'None':
                        x_axis_min_ter = st.number_input(f'Set Minimum X-Axis Value for {tertiary_curve}', value=well_data[tertiary_curve].min(), key=f'min_{tertiary_curve}_{i}')
                        x_axis_max_ter = st.number_input(f'Set Maximum X-Axis Value for {tertiary_curve}', value=well_data[tertiary_curve].max(), key=f'max_{tertiary_curve}_{i}')
                    else:
                        x_axis_min_ter = None
                        x_axis_max_ter = None

                    if primary_curve:
                        fig.add_trace(go.Scatter(x=well_data[primary_curve], y=well_data.index, name=primary_curve, line=dict(color='blue')), row=i, col=1)
                        fig.update_xaxes(title_text=primary_curve, row=i, col=1, range=[x_axis_min, x_axis_max])
                        
                    if secondary_curve != 'None':
                        fig.add_trace(go.Scatter(x=well_data[secondary_curve], y=well_data.index, name=secondary_curve, line=dict(color='red')), row=i, col=1)
                        fig.update_xaxes(title_text=f"{primary_curve} and {secondary_curve}", row=i, col=1, range=[min(x_axis_min, x_axis_min_sec), max(x_axis_max, x_axis_max_sec)])

                    if tertiary_curve != 'None':
                        fig.add_trace(go.Scatter(x=well_data[tertiary_curve], y=well_data.index, name=tertiary_curve, line=dict(color='green')), row=i, col=1)
                        fig.update_xaxes(title_text=f"{primary_curve}, {secondary_curve}, and {tertiary_curve}", row=i, col=1, range=[min(x_axis_min, x_axis_min_sec, x_axis_min_ter), max(x_axis_max, x_axis_max_sec, x_axis_max_ter)])

                    if primary_curve.startswith('RES') or (secondary_curve != 'None' and secondary_curve.startswith('RES')) or (tertiary_curve != 'None' and tertiary_curve.startswith('RES')):
            ...
            if zones:
                for zone in zones:
                    fig.add_hrect(
                        ...
                        annotation_text=zone['label'], annotation_position="top left"
                    )
            ...
            fig.update_layout(height=800, showlegend=True, xaxis_title='Value', yaxis_title='Depth', yaxis_autorange='reversed')
            st.plotly_chart(fig, use_container_width=True)

        with st.expander('Average Log Plot'):
            st.write("Select the curve and the interval for averaging.")
            
            avg_curve = st.selectbox('Select Curve for Averaging', columns, key='avg_curve')
            avg_interval = st.number_input('Set Averaging Interval (Depth units)', value=1000, min_value=1, step=1)
            
            if avg_curve:
                averaged_data = calculate_log_average(well_data, avg_curve, interval=avg_interval)
                fig_avg = go.Figure()
                fig_avg.add_trace(go.Scatter(x=averaged_data[avg_curve], y=averaged_data['Interval'], mode='lines+markers', name=f'Average of {avg_curve}'))
                fig_avg.update_layout(height=600, yaxis={'title': 'DEPTH', 'autorange': 'reversed'}, xaxis={'title': avg_curve}, title=f'Average Log Plot for {avg_curve}')
...
def main():
    ...
    if combined_well_data is not None:
        ...
        if st.checkbox("Add Zonation"):
            num_zones = st.number_input("Number of Zones", min_value=1, max_value=10, value=1)
            for i in range(num_zones):
                st.subheader(f"Zone {i+1}")
                label = st.text_input(f"Label for Zone {i+1}", value=f"Zone {i+1}")
                start = st.number_input(f"Start Depth for Zone {i+1}", value=combined_well_data.index.min())
                end = st.number_input(f"End Depth for Zone {i+1}", value=combined_well_data.index.max())
                zones.append({'label': label, 'start': start, 'end': end})

        cutoffs = []
        if st.checkbox("Add Cutoffs"):
            num_cutoffs = st.number_input("Number of Cutoffs", min_value=1, max_value=10, value=1)
        ...
        option = st.sidebar.selectbox('Select an option:', ['Missing Data', 'Log Plot', 'Average Plot'])
        ...
        if option == 'Missing Data':
            ...
            st.write("Select the curve and the interval for averaging.")
            avg_curve = st.selectbox('Select Curve for Averaging', combined_well_data.columns, key='avg_curve')
            avg_interval = st.number_input('Set Averaging Interval (Depth units)', value=1000, min_value=1, step=1)
            
            if avg_curve:
                averaged_data = calculate_log_average(combined_well_data, avg_curve, interval=avg_interval)
                fig_avg = go.Figure()
                fig_avg.add_trace(go.Scatter(x=averaged_data[avg_curve], y=averaged_data['Interval'], mode='lines+markers', name=f'Average of {avg_curve}'))
                fig_avg.update_layout(height=600, yaxis={'title': 'DEPTH', 'autorange': 'reversed'}, xaxis={'title': avg_curve}, title=f'Average Log Plot for {avg_curve}')
                st.plotly_chart(fig_avg, use_container_width=True)
